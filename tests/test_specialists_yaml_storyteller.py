...
def test_agents_include_contains_storyteller() -> None:
    ...
    assert "specialists:specialists/storyteller" in include_list, (
        "agents.include must contain 'specialists:specialists/storyteller'"
    )
...
def test_raw_line_present_in_file() -> None:
    """Acceptance criterion: raw line '    - specialists:specialists/storyteller' is in the file."""
    content = SPECIALISTS_YAML_PATH.read_text(encoding="utf-8")
    assert "    - specialists:specialists/storyteller" in content, (
        "The exact line '    - specialists:specialists/storyteller' must appear in the file"
    )
