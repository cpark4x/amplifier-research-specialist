...
def test_status_summary_shows_six_planned():
    """Status summary should say '6 planned', not '7 planned'."""
    content = read_backlog()
    ...
    assert "6 planned" in summary_line, (
        f"Expected '6 planned' in summary line, got: {summary_line}"
    )
    assert "7 planned" not in summary_line, (
        f"Old '7 planned' still present in summary line: {summary_line}"
    )
