...
- [ ] **Implement hook-specialist-narration for real** — replace the stub `mount()` with actual hook behavior that emits narration lines during specialist delegation chains. Remove the contradictory instruction that tells the coordinator not to emit narration itself, OR keep the instruction as fallback guidance and have the hook be the primary mechanism. (Severity: medium — narration works today via coordinator compliance but is fragile)
- [ ] **Clarify ambiguity boundary for "tell me about" prompts** — decide whether casual informational questions should trigger the specialist chain or not. Options: (a) add explicit guidance in specialists-instructions.md for ambiguous phrasing, (b) accept that casual questions bypass the chain and document this as intentional behavior, (c) add a "when in doubt, chain" rule. (Severity: medium — affects user experience for the most common prompt pattern)
- [ ] **Investigate Parsed: line inconsistency** — determine why `Parsed:` appeared in Test 3 (competitive → writer) but not Test 1 (researcher → writer). May be a writer output formatting issue specific to certain input types. (Severity: low — doesn't break the chain, but affects machine parseability)
