...
|---|------|------|-------|--------|--------|-----------|
| 29 | Writer: OUTPUT CONTRACT compliance for informal registers | 02 | Chris | S | M | `audience: myself` run produced no `Parsed:` line and no structural blocks — OUTPUT CONTRACT violation. Register table's "skip formality" language was interpreted as permission to skip structural requirements. Consider adding WRONG/RIGHT example to OUTPUT CONTRACT block showing structural blocks required even for informal registers. *(from test log 2026-03-06-writer-quality-fixes-validation)* **Update 2026-03-09:** Fix in commit `0508f95` did not hold — OUTPUT CONTRACT block is present and explicit in the spec, but the model still ignores it when register is informal (all structural blocks absent). Stronger enforcement needed: WRONG/RIGHT example pair in OUTPUT CONTRACT showing informal prose with blocks intact; plus a Stage 5 structural self-check that fires regardless of register. *(from test log 2026-03-09-writer-informal-register)* |
...
| 31 | Prioritizer: add structured output block (`PRIORITY OUTPUT` header) | 10 | Chris | S | M | Every other specialist produces a machine-parseable header block (STORY OUTPUT, RESEARCH OUTPUT, ANALYSIS OUTPUT). Prioritizer returns markdown prose — breaks pipeline parsability and violates the output contract pattern. Required before Prioritizer can be used as a pipeline stage rather than standalone tool. *(from test log 2026-03-09-prioritizer-spot-test)* **Update 2026-03-09:** Spec-based fix attempted in `01e137a` (Stage 0 emit-immediately + WRONG/RIGHT examples). Fix did not hold — Prioritizer still returns markdown prose per routing validation test log. Same root cause as Writer/Storyteller/Researcher. Architectural fix required: **prioritizer-formatter specialist** (#33). |
...
| 33 | Prioritizer-formatter specialist | 10 | Chris | S | H | Prioritizer produces good ranked output but no `PRIORITY OUTPUT` structured block. Instruction-based fix (#31, `01e137a`) did not hold — same LLM generation prior issue as Researcher/Storyteller/Writer. Architectural fix: a prioritizer-formatter specialist that receives raw Prioritizer prose + original items list and wraps output in canonical `PRIORITY OUTPUT` block (Goal, Framework, RANKINGS, UNRANKABLE ITEMS, QUALITY THRESHOLD RESULT). Fourth application of the formatter pattern — now the established architecture for output compliance. *(from test log 2026-03-09-routing-validation)* |
