...
| 37 | Writer: hedging-to-source-tier mapping | 02 | Chris | S | H | Writer applies blanket epistemic caution ("reportedly," "available analysis indicates") regardless of source tier. A Nature RCT should read as established fact, not a tentative suggestion. Add explicit mapping: primary tier = state directly, secondary = "research indicates," tertiary = "survey data suggests." Prevents hedging accumulation through multi-agent chains where each agent independently adds caution. *(test log 2026-03-18-specialists-vs-direct-rto-research)* |
| 38 | Writer: decision-support template for executive audiences | 02 | Chris | S | M | When `audience=executive`, add a "Scenario Implications" section template: "If you choose X → expect Y." Concrete outcomes per policy option, not conceptual reframes. A/B test showed direct approach produced better exec output because it gave three concrete scenarios with expected outcomes rather than a conceptual closing. *(test log 2026-03-18-specialists-vs-direct-rto-research)* |
