...
**Owner:** Chris Park  
**Last Updated:** March 19, 2026 (v5.2)  
...
*Recommended ship order for open items: #49 (S/H, ~0.4 pts quality lever) → #46 (M/H, breadth for survey questions) → #50 (S/M) → #44 (S/M, unblocked by #45) → #43 (S/M) → #40 (S/M)*
...
| 44 | Writer-formatter: graceful degradation when source material unavailable | 02 | Chris | S | M | **Unblocked by #45.** Writer-formatter broke in ad-hoc mode — requested original RESEARCH OUTPUT as second input, which the coordinator couldn't re-send (~15K tokens). Two-path doctrine (#45) now skips writer-formatter in ad-hoc mode, but formatter should still degrade gracefully when source material not provided: format what you have (extract claims from writer's inline `> *Sources: S1, S2*` attributions, produce partial CITATIONS block, skip S-numbered extraction). *(test log 2026-03-19-retirement-research-specialists-vs-direct)* |
