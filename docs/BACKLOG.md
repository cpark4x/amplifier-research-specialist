...
| 36 | Pipeline token/cost optimization | 01/02/03/08 | — | M | H | Deep chain spawns 5+ agent sessions sequentially; formatters add 3–4 sessions that may not need heavy models. Four levers: (1) **Formatter consolidation** — run formatting as tool call within preceding specialist's session instead of spawning new agent, cutting 3–4 sessions from deep chain; (2) **Model routing** — route formatters to `fast` tier (Haiku-class) since they extract/reformat, not reason; (3) **Smart depth detection** — reduce accidental deep-chain triggers via better UX routing; (4) **Research caching** — cache ResearchOutput with TTL to cut repeat queries to near-zero cost. Levers 1–2 are low-hanging fruit; 3 is UX; 4 is higher effort/higher payoff for teams. *(from pitch discussion 2026-03-16)* |
