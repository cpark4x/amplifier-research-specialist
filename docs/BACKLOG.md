...
| 26 | Clarify ambiguity boundary for casual prompts in chain instructions | 01/02/04/08 | Chris | S | M | Smoke test showed "Tell me about spaced repetition" bypassed the specialist chain entirely — coordinator answered conversationally despite Rule 1 saying it must never generate specialist-domain content itself. The coordinator's judgment was defensible (casual phrasing, general knowledge), but creates an inconsistency. Options: (a) add explicit guidance for "tell me about" style prompts, (b) accept that casual questions don't trigger the chain and document as intentional, (c) add a "when in doubt, chain" rule. *(from smoke test 2026-03-05)* |
