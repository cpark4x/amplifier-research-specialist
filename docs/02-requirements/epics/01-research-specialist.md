...
- [ ] **Researcher output format compliance** — Chain test (2026-03-03, ai-pair-programming) found the Researcher produced rich narrative markdown with sub-question sections and embedded tables instead of the canonical `RESEARCH OUTPUT` block format specified in Stage 7 (Synthesizer). The narrative is high quality for human readers but not machine-parseable by the Data Analyzer without manual reformatting. Decision needed: (a) is narrative output intentional — in which case the pipeline needs an explicit adapter step, or (b) is it spec drift — in which case Stage 7 needs a compliance fix. *(from test log 2026-03-03)*
