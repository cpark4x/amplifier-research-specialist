...
- [x] Should the chain encode Researcher as a default first step for product comparisons? **Resolved 2026-03-02** — implemented as `competitive-analysis-brief-researched` recipe (3-step: Researcher → competitive-analysis → writer). Caller chooses 2-step (fast) or 3-step (rigorous) based on need.
- [x] Should the Writer surface a "claims to verify" block when the upstream `CompetitiveAnalysisOutput` contains highly specific numerical claims with no cited source? **Resolved 2026-03-02** — Writer now produces `CLAIMS TO VERIFY` block (Block 4) for all `unrated` claims containing specific values when input type is `analyst-output`. Smoke tested and confirmed.
- [x] Should the `competitive-analysis → writer` chain be encoded as a single invokable recipe so callers don't have to manually dispatch both agents? **Resolved 2026-03-02** — implemented as `competitive-analysis-brief` recipe (2-step chain). Both recipes pass validation.
